...
def getSetups(car_code, currentTrackBaseName, currentTrackLayout):
    try:
        ...
        ac.log('BoxRadio: error requesting setups from box api: ' + traceback.format_exc())
...
def downloadSetup(setup_id, setup_file_name, car_ac_code, track_baseName, track_layout, refreshSetupsListingTable):
    ...
    try:
        ...
        ac.console('BoxRadio: failed at r = requests.get(url)')
        r = {}
        r['status_code'] = ''
    ...
    if r.status_code == 200:
        try:
            ...
            ac.log('BoxRadio: could not find folder to save')

    else:
        ac.log('BoxRadio: setupid: ' + setup_id + 'error while downloading')
...
def getAllSetupsFromFolder(car_ac_code, track_baseName):
    ...
    try:
        ...
        ac.log('BoxRadio: getAllSetupsFromFolder: all setups in folder = ' + str(allSetupFiles))
    except:
        ac.log('BoxRadio: getAllSetupsFromFolder failed at allSetupFiles = os.listdir')
        return []

    # Build a new list without all files downloaded by the app (files starting with box)
    allSetupFilesNotTSM = []

    for setupFile in allSetupFiles:
        if 'box-ac' not in setupFile:
            allSetupFilesNotTSM.append(setupFile)

    ac.log('BoxRadio: all setups in folder not box: ' + str(allSetupFilesNotTSM))
...
def uploadSetup(userTSMId, ac_version, user_steamId, filename, trim, baseline, car_id, track_id, car_ac_code, track_baseName, track_layout):
    ...
    ac.log('BoxRadio: uploadSetup params: ' + str(filename) + ', ' + str(trim) + ', ' +  str(baseline) + ', ' + str(car_id) + ', ' + str(track_id))

    filepath = get_personal_folder() + r'\Assetto Corsa\setups' + '\\' + car_ac_code + '\\' + track_baseName + '\\' + filename
    ac.log('BoxRadio: uploadSetup filepath: ' + str(filepath))
    ...
    if r.status_code == 200:
        ac.log('BoxRadio: upload request success! Status code: ' + str(r.status_code))
        return True
    else:
        ac.log('BoxRadio: upload request failed! Status code: ' + str(r.status_code))
        ac.log('BoxRadio: upload request failed! text: ' + str(r.text))
        ac.log('BoxRadio: upload request failed! content: ' + str(r.content))
        return False
...
def getUserSetups(userTSMId, car_id, track_id):
    try:
        ...
        ac.log('BoxRadio: error requesting user setups from box api: ' + traceback.format_exc())
...
def getSetupDetails(setupId, callback):
    try:
        ...
        ac.log('BoxRadio: error requesting user setup details from box api: ' + traceback.format_exc())
...
def updateSetup(car_ac_code, track_baseName, file_name, setup_id, car_id, track_id, sim_version, trim, baseline, best_time, comments, callback):
    ...
    ac.log('BoxRadio: updateSetup filepath: ' + str(filepath))
    ...
    if r.status_code == 200:
        ac.log('BoxRadio: update request success! Status code: ' + str(r.status_code))
        returnMessage = 'Setup successfully updated'
    else:
        ac.log('BoxRadio: update request failed! Status code: ' + str(r.status_code))
        ac.log('BoxRadio: update request failed! text: ' + str(r.text))
        ac.log('BoxRadio: update request failed! content: ' + str(r.content))
...
def get_personal_folder():
    ...
    if dll.SHGetSpecialFolderPathW(None, buf, 0x0005, False):
        ...
        raise Exception('BoxRadio: Could not find "Documents" folder')
...
def get_ac_version_from_api():
    ...
    if r.status_code == 200:
        try:
            ...
            ac.log('BoxRadio: get_ac_version_from_api failed at request_json = r.json()')
            ac_current_version = False
    else:
        ac.log('BoxRadio: get_ac_version_from_api failed. status_code = ' + str(r.status_code))
        ac_current_version = False
...
def get_carid_from_api(ac_code):
    ...
    if r.status_code == 200:
        ...
        ac.log('BoxRadio: get_carid_from_api failed. status_code = ' + str(r.status_code))
        carId = False
...
def get_trackid_from_api(ac_code):
    ...
    if r.status_code == 200:
        ...
        ac.log('BoxRadio: get_trackid_from_api failed. status_code = ' + str(r.status_code))
        trackId = False
...
def getUserTSMIdWithSteamID(steamID):
    ...
    if r.status_code == 200:
        try:
            ...
            ac.log('BoxRadio: getUserTSMIdWithSteamID failed at request_json = r.json() = ')
            userTSMId = False
    else:
        ac.log('BoxRadio: getUserTSMIdWithSteamID failed. status_code = ' + str(r.status_code))
        userTSMId = False
...
def checkIfUserExistsOnTSM(userSteamID):
    ac.log('BoxRadio: checkIfUserExistsOnTSM userSteamID = ' + str(userSteamID))
    userExists = False
    ...
    if r.status_code == 200:
        ac.log('BoxRadio: checkIfUserExistsOnTSM status = 200')
        try:
            request_json = r.json()

            userExists = True
        except:
            ac.log('BoxRadio: checkIfUserExistsOnTSM failed at request_json = r.json()')
